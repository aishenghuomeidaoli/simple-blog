...
from . import main
from .forms import NameForm, ForgetpasswordForm, EditProfileAdminForm, ResetpasswordForm, PostForm, CommentForm, EditProfileForm
from ..import db
...
def index():
	...
	form.validate_on_submit():
		...
		#current_user不是真正对象，所以调用_get_current_object()
		db.session.add(post)
...
def post(id):
	...
	if page==-1:
		...
		current_app.config['FLASK_COMMENTS_PER_AGE']+1
	pagination=post.comments.order_by(Comment.timestamp.asc()).paginate(page,
		per_page=current_app.config['FLASK_COMMENTS_PER_AGE'],error_out=False)
...
def forget_password():
	...
	if form.validate_on_submit():
		...
		if user is not None:
			...
			send_email(form.email.data,'重置密码','mail/resetpassword',user=user,token=token)
			flash('密码重置邮件已发送至邮箱,请查收')
			return redirect(url_for('.index'))
		else:
			flash('无此账户')
